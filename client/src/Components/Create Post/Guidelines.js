...
function Guidelines() {
  return (
    <div className="bg-white shadow-base max-w-[20rem] pb-2 my-2 rounded absolute left-2 top-[12%] mx-4">
      {/* title */}
      <div className="w-full bg-primary-light p-3 py-2 rounded-t">
        ...
        <p className="text-white text-xl inline"> Guidelines</p>
      </div>
      ...
      <div>
        {guidelines.map((guidline, i) => (
          <div
            className={`${
              guidelines[i + 1] ? "border-b border-[#cfcfcf] text-left" : ""
            } py-3 text-left`}
            key={i}
          >
            <p className="px-3 text-sm">{guidline}</p>
          </div>
        ))}
      </div>
    </div>
